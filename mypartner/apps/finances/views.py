...
class ConceptoDetailView(APIView):
    ...
    def delete(self, request, group_id, concept_id):
        ...
        if has_movements:
            return Response(
                {
                    'detail': 'Este concepto tiene movimientos asociados.',
...
class ConceptoDeleteWithMovementsView(APIView):
    ...
    def post(self, request, group_id, concept_id):
        accion = request.data.get('accion')  # 'eliminar_movimientos' | 'mantener_movimientos'
        if accion not in ('eliminar_movimientos', 'mantener_movimientos'):
            return Response(
                {'detail': 'Acción inválida. Use "eliminar_movimientos" o "mantener_movimientos".'},
...
class MovimientoListCreateView(APIView):
    ...
    def get(self, request, group_id):
        ...
        serializer = MovimientoSerializer(page, many=True)
    ...
    def post(self, request, group_id):
        ...
        if concepto and str(concepto.grupo_id) != str(group_id):
            return Response(
                {'detail': 'El concepto no pertenece a este grupo.'},
                status=status.HTTP_400_BAD_REQUEST,
            )

        movimiento = serializer.save(usuario=request.user, grupo=grupo)

        tipo_lower = movimiento.tipo.lower()
        concepto_nombre = movimiento.concepto.nombre if movimiento.concepto else 'Desconocido'
        titulo = (
            f'Se generó un gasto por ${movimiento.monto:,} de {request.user.username} por {concepto_nombre}'
            if tipo_lower == 'gasto'
...
class FinanciasDashboardView(APIView):
    ...
    def get(self, request, group_id):
        ...
        grafico = []
        if gastos_por_concepto.count() <= 5:
            for item in gastos_por_concepto:
                grafico.append({
                    'concepto': item['concepto__nombre'] or 'Desconocido',
                    'total': item['total'],
                })
        else:
            top5 = list(gastos_por_concepto[:5])
            otros_total = sum(item['total'] for item in gastos_por_concepto[5:])
            grafico = [
                {'concepto': item['concepto__nombre'] or 'Desconocido', 'total': item['total']}
                for item in top5
            ]
            if otros_total > 0:
...
class PresupuestoListCreateView(APIView):
    ...
    def get(self, request, group_id):
        ...
        serializer = RegistroPresupuestoSerializer(registros, many=True)
        return Response({'registros': serializer.data, 'total': total})
    ...
    def post(self, request, group_id):
        ...
        registro = serializer.save(grupo=grupo)
...
class PresupuestoDetailView(APIView):
    ...
    def patch(self, request, group_id, budget_id):
        try:
            registro = RegistroPresupuesto.objects.select_related('concepto').get(
                id=budget_id, grupo_id=group_id
            )
